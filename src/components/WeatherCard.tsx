...
export function WeatherCard(props: { data: WeatherData }) {
  const { data } = props
  return (
    <div className='cardContainer'>
      <div className='card'>
        <p className='city'>{data.fxDate}</p>
        <p className='weather'>
          {data.textDay}/{data.windDirDay}
        </p>
        <svg
          className='weather'
          version='1.1'
        ...
        <p className='temp'>
          {(Number(data.tempMin) + Number(data.tempMax)) / 2}°
        </p>
        <div className='minmaxContainer'>
          <div className='min'>
            <p className='minHeading'>最低温</p>
            <p className='minTemp'>{data.tempMin}°</p>
          </div>
          <div className='max'>
            <p className='maxHeading'>最高温</p>
            <p className='maxTemp'>{data.tempMax}°</p>
          </div>
        </div>
      </div>
    </div>
